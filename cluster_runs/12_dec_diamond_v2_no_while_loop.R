...
# initial_gene_list <- append(added_genes_list, initial_seeds
# added_genes_list <- list()

diamond_iteration <- function(seeds, s, PPI, alpha) { # single iteration, starting seed are starting seeds for the iteration
  ...
  all_seeds <- s  # after every iteration one diamond gene should be added to all seeds
  ...
  # number_of_all_links <- c() # after every iteration a k(degree) of added diamond gene is added to this vector
    ...
    CS_pvals <- mapply(ks = remaining_genes_ks, 
                       k = remaining_genes_k,
                       N = N,
                       s = s,
                       s0 = s0,
    ...
    diamond_genes <- new_gene
    pvalues <- p_value
    number_of_links_to_seeds <- new_gene_ks
    number_of_all_links <- new_gene_k
  ...
  results_list <- list(added_genes = added_genes_df) # only one gene
  
  return(results_list)
  
}

input_seeds <- seeds_10_weeks$Gad2
start_seeds <- seeds_10_weeks$Gad2

added_genes_list <- list()

for (i in 1:5){
        iteration_output <- diamond_iteration(seeds = start_seeds, s = input_seeds, PPI = string_igraph, alpha = 10)
        input_seeds <- append(input_seeds, iteration_output$added_genes$gene)
        added_genes_list[[i]] <- iteration_output
        }
...
run_diamond <- function(alpha, iteration_no){
                result <- sapply(X = seeds_10_weeks[1:3], # skips PV and SST 
...
b <- Sys.time()
all_cells_diamond_1000x_alpha1_20 <- mclapply(X = c(1:20),
                                               FUN = run_diamond, 
                                               iteration_no = 1000,
                                               mc.preschedule = TRUE, 
                                               mc.set.seed = TRUE,
...
a <- Sys.time()
a-b
# saveRDS(all_cells_diamond_1000x_alpha1_20, "all_cells_diamond_1000x_alpha1_20.rds")
