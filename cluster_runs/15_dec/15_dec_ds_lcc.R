...
degrees <- degree(entire_PPI_network_igraph, v = V(entire_PPI_network_igraph), loops = TRUE)
...
while (i >= min(degrees)){
        ...
        k <- i
        if(length(x[x %in% k:i]) < 100){
                while (length(x[x %in% k:i]) < 100){
                        k <- k-1
                }
...
similar_vertices_ds <- function(x, ENTREZ_vector){
        ...
        all_distances <- distances(entire_PPI_network_igraph, v = random_vertices_vec, to = random_vertices_vec, weights = NA) #calculate the shortest paths between all pairs of vertices
...
multiple_random_ds_vecs <- function(cell_type, n_randomizations){   # x = vector with ENTREZ gene IDs
        
        ds_list <- mclapply(1:n_randomizations,
                            FUN = similar_vertices_ds,
                            ENTREZ_vector = seeds_10_weeks[[cell_type]],
                            mc.preschedule = TRUE, 
                            mc.set.seed = TRUE,
                            mc.silent = FALSE, 
...
b <- Sys.time()
a <- multiple_random_ds_vecs(n_randomizations = as.numeric(args[1]), cell_type = args[2])
c <- Sys.time()
c-b

saveRDS(a, paste0("random_distances_", args[1], "_", args[2], ".rds"))



# get mean ds of entrez vec and a z score:

ds_zscore <- function(ENTREZ, random_ds_list){ # ENTREZ: vector of entrez gene IDs, random_ds_list: list with vecs with ds values of random genes
        
        all_distances <- distances(entire_PPI_network_igraph, v = ENTREZ, to = ENTREZ, weights = NA) 
        all_distances_removed_zeros <- replace(all_distances, all_distances == 0, NA)
        ds_vec <- rowMins(all_distances_removed_zeros, na.rm = TRUE)
        ds_vec <- ds_vec[which(!is.infinite(ds_vec))]
        
        mean_ds <- mean(ds_vec)
        
        #get random mean ds values from random_ds_list
        
        random_mean_ds <-sapply(X = random_ds_list, FUN = mean, simplify = TRUE, USE.NAMES = FALSE)
        
        z_score <- (mean_ds - mean(random_mean_ds))/sd(random_mean_ds)
        return(z_score)
}

ds_zs <- ds_zscore(random_ds_list = a,
                        ENTREZ = seeds_10_weeks[[args[2]]])

################## LCCs


similar_vertices_LCCs <- function(x, ENTREZ_vector){
        x <- degree_df %>% filter(vertex %in% ENTREZ_vector)
        x$interval <- factor(x$interval, levels = unique(x$interval))
        y <- as.vector(table(x$interval))
        names(y) <- as.character(levels(x$interval))
        ...
        random_subgraph <- induced_subgraph(entire_PPI_network_igraph, vids = random_vertices_vec, impl = "auto")
...
#repeat similar_vertices_LCCs() n_randomizations times to get a vector  of random LCC sizes

multiple_random_LCCs <- function(cell_type, n_randomizations){   # x = vector with ENTREZ gene IDs
        
        LCC_vec <-  mclapply(1:n_randomizations, 
                 FUN = similar_vertices_LCCs, 
                 ENTREZ_vector = seeds_10_weeks[[cell_type]],
                 mc.preschedule = TRUE, mc.set.seed = TRUE,
                 mc.silent = FALSE, mc.cores = getOption("mc.cores", 10L),
                 mc.cleanup = TRUE, mc.allow.recursive = TRUE, affinity.list = NULL)
        
        return(unlist(LCC_vec))
}

b <- Sys.time()
random_LCCs <- multiple_random_LCCs(n_randomizations = args[1], cell_type = args[2])
c <- Sys.time()
c-b

saveRDS(random_LCCs, paste0("random_LCCs_", args[1], "_", args[2], ".rds"))


LCC_zscore <- function(ENTREZ, random_LCCs, iterations){ # ENTREZ: vector of entrez gene IDs, random_ds_list: list with vecs with ds values of random genes
        
        # get LCC of seeds:
        
        ENTREZ_graph <- induced_subgraph(entire_PPI_network_igraph, vids = ENTREZ, impl = "auto")
        Cs_ENTREZ_graph <- components(ENTREZ_graph)
        observed_LCC <- max(Cs_ENTREZ_graph$csize)
        
        LCC_zscore <- (observed_LCC - mean(random_LCCs))/sd(random_LCCs)
        LCC_pval <- sum(random_LCCs >= observed_LCC)/as.numeric(iterations)
        return(c(LCC_pval, LCC_zscore))
    
}

z_and_p <- LCC_zscore(random_LCCs = random_LCCs,
        ENTREZ = seeds_10_weeks[[args[2]]], 
        iterations = as.numeric(args[1])
)

write.table(file = paste0("LCC_stats_", args[2],"_", args[1], "_iterations.txt"), 
            paste("LCC Z zcore is", z_and_p[2], 
                  "LCC p value is", z_and_p[1], 
                  "and distance z_score is", ds_zs),
            quote = F,
            row.names = F,
            col.names = F)
